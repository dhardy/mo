...
<pre><code>//Standard arithmatic, where T is one of ... (some atomic types), OP is one of "plus" || "minus" || "multiply" || "divide" || "modulus" || ... (check what makes sense):
...
Possible examples:
<pre><code>  template(T) T if (Bool cond, T tVal, T fVal) {...}	// D
